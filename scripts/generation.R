# this file is copied from irace's source code, see https://cran.r-project.org/web/packages/irace/index.html
                           ...
                           forbidden = NULL, repair = NULL)
{
  ...
  for (idxConfiguration in seq_len(nbConfigurations)) {
    forbidden.retries <- 0
    while (forbidden.retries < maxRetries) { #nguyen
    ...
    if (forbidden.retries >= maxRetries) { #nguyen
